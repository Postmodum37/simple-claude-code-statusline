...
- Rate-limit color (`usageColor()`) is pace-based: projected end-of-window usage = used% × window / elapsed, where elapsed is derived from `resets_at` (5h = 18000s, 7d = 604800s) and clamped to ≥25% of the window (`paceMinElapsedFraction`). Pace bands: <85 ok, <100 warn, <120 high, else crit. Pace may relax the raw percent band (`getSemanticColor()`) freely but raise it by at most one step (so ≤50% used is never worse than warn, ≤75% never worse than high); floored at high/crit at 75%/90% used. Falls back to percent bands without a reset time.
