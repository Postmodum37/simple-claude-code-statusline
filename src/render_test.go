...
func TestUsageColor(t *testing.T) {
	...
	tests := []struct {
		...
		{"40% used, 1h elapsed → projected 160% but raw band green, capped at yellow", 40, fiveHourWindowSecs, 4 * h, cWarn},
		{"70% used, 1h elapsed → projected 280%, raw band yellow, capped at orange", 70, fiveHourWindowSecs, 4 * h, cHigh},
		...
		{"10% used, 2 minutes in → elapsed clamped to 25%, projected 40% → green", 10, fiveHourWindowSecs, 5*h - 2*time.Minute, cOK},
		{"24% used, 2 minutes in → elapsed clamped to 25%, projected 96% → yellow", 24, fiveHourWindowSecs, 5*h - 2*time.Minute, cWarn},
		{"3% used, 2 minutes in → projected 12% → green", 3, fiveHourWindowSecs, 5*h - 2*time.Minute, cOK},
		{"screenshot: 14% used, 4h14m left → elapsed clamped to 25%, projected 56% → green", 14, fiveHourWindowSecs, 4*h + 14*time.Minute, cOK},
		{"screenshot: 7d 27% used, 5d17h left → projected 108%, raw band green, capped at yellow", 27, sevenDayWindowSecs, 5*24*h + 17*h, cWarn},
		{"7d: 30% used, 3 days elapsed → projected 70%", 30, sevenDayWindowSecs, 4 * 24 * h, cOK},
		{"7d: 60% used, 3 days elapsed → projected 140%, raw band yellow, capped at orange", 60, sevenDayWindowSecs, 4 * 24 * h, cHigh},
		{"7d: 80% used, 3 days elapsed → projected 187%, raw band orange → red", 80, sevenDayWindowSecs, 4 * 24 * h, cCrit},
		{"reset time in the past → elapsed clamped to full window, projected 70% → green", 70, fiveHourWindowSecs, -10 * time.Minute, cOK},
	}
	for _, tt := range tests {
